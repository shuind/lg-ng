  ...
  return (
    <div className="relative" data-chat-popover-keepopen="true">
      ...
      >
        ...
        <span className="hidden sm:inline">{selected ? selected.command : "写作动作"}</span>
        <ChevronDown className={cn("h-3 w-3 transition", open && "rotate-180")} />
      </button>

      {open && (
        <div className="absolute bottom-9 left-0 z-40 w-64 rounded-lg border border-border/70 bg-popover p-1.5 text-popover-foreground shadow-lg">
          {WORKFLOW_ACTIONS.map((item) => (
            <button
              ...
              className={cn(
                "flex w-full items-center gap-2 rounded-md px-2 py-2 text-left transition hover:bg-secondary",
                workflowAction === item.id && "bg-secondary",
              )}
            >
              <item.Icon className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
              <span className="min-w-0 flex-1">
                <span className="block text-[12px] font-medium text-foreground">{item.command}</span>
              </span>
            </button>
          ))}
        </div>
