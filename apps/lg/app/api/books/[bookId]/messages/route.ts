...
export async function GET(
  ...
  try {
    ...
    const messages = await listThreadMessages(bookId, thread.id)
...
export async function POST(
  ...
  try {
    ...
    const constraintIds = parseConstraintIds(body.constraintIds)
    const skillIds = parseSkillIds(body.skillIds)
    const temporaryConstraints = parseTemporaryConstraints(body.temporaryConstraints)
    ...
    const targetThreadId = typeof threadId === "string" && threadId.trim() ? threadId : defaultThread.id
    const targetThread = await getThread(bookId, targetThreadId)
    if (!targetThread || targetThread.status !== "active") {
      return NextResponse.json({ error: "当前线程不可发送消息" }, { status: 409 })
    }
    const selectedSkills = await resolveSkillSummaries(bookId, skillIds)
