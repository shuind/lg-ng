...
- 写入 \`canon/\` 前必须说明：写哪些文件、正典还是候选、是否影响已有设定、是否保留来源。
...
写入 \`canon/\` 是受保护动作：先说明目标文件、正典/候选状态、影响范围、来源保留和 diff 方案，再等待确认。
`;
...
落盘是受保护动作。原则：先说明方案 -> 取得确认 -> 写入 -> 留 git diff。
...
5. 用户确认后才写 \`canon/\`，并更新 \`NOVEL.md\` 指针。
`;
