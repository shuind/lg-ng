...
import { listImportedMaterials } from "@/lib/server/import-store"
...
export async function GET(
  ...
  try {
    ...
    const [chapters, outlines, threads, bundle, cards, importedMaterials, responseConstraints] = await Promise.all([
      listChapters(bookId),
      listOutlineFiles(bookId),
      listThreads(bookId),
      getThreadBundle(bookId, defaultThread.id),
      listSettingCards(bookId),
      listImportedMaterials(bookId),
